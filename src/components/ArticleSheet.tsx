...
export default function ArticleSheet({ isOpen, onClose, post }: ArticleSheetProps) {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-[300] bg-white text-black rounded-t-[40px] shadow-[0_-10px_50px_rgba(0,0,0,0.2)] max-h-[94vh] flex flex-col transition-transform duration-600 ease-out ${isOpen ? 'translate-y-0 pointer-events-auto' : 'translate-y-full pointer-events-none'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-8 pb-24 pt-10 lg:px-16">
        <h1 className="font-cormorant text-[clamp(28px,7vw,42px)] font-bold leading-tight text-zinc-950 mb-4">{post.title}</h1>
        <p className="text-[11px] font-bold text-amber-600 uppercase tracking-[.2em] italic border-l-4 border-amber-500 pl-4 mb-10">{post.tagline}</p>
        ...
        <div className="prose prose-zinc max-w-none" dangerouslySetInnerHTML={{ __html: post.body || '' }} />
        ...
        <div className="mt-16 bg-zinc-950 text-white rounded-[32px] p-10">
          ...
          <button className="w-full py-5 bg-amber-600 rounded-2xl text-[11px] font-extrabold tracking-[.2em] uppercase hover:bg-amber-700 active:scale-95 transition-all pointer-events-auto cursor-pointer">Download Guide — $15</button>
        </div>
      </div>
    </div>
  );
