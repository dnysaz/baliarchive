  ...
  return (
    <div className={`fixed inset-0 z-[300] bg-white text-black flex flex-col transition-transform duration-500 ease-out ${isOpen ? 'translate-x-0 pointer-events-auto' : 'translate-x-full pointer-events-none'}`}>
      <div className="shrink-0 flex items-center gap-4 px-6 border-b border-black/[.07] pt-[max(20px,env(safe-area-inset-top))] pb-5">
        <button 
          className="w-10 h-10 rounded-full bg-black/5 flex items-center justify-center text-zinc-500 appearance-none outline-none cursor-pointer pointer-events-auto"
          ...
          {savedPosts.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {savedPosts.map(post => (
                <button 
                  key={post.id} 
                  className="relative group overflow-hidden rounded-[24px] aspect-[3/4] appearance-none outline-none text-left cursor-pointer pointer-events-auto"
