...
async function getTermsSettings() {
  try {
    ...
    return {
      title: settings?.termsTitle || 'Terms & Conditions',
      content: settings?.termsContent || '<p>Terms and Conditions go here.</p>',
    };
  } catch (error) {
    console.error('Failed to fetch terms settings:', error);
    return {
      title: 'Terms & Conditions',
      content: '<p>Terms and Conditions go here.</p>',
    };
  }
}

export async function generateMetadata(): Promise<Metadata> {
  const settings = await getTermsSettings();
  return {
    title: `${settings.title} — BaliArchive`,
    description: 'Terms and Conditions of BaliArchive.',
  };
}
    ...
    <div className="flex flex-col h-full bg-white text-zinc-900 overflow-y-auto selection:bg-amber-100 selection:text-amber-900">
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto px-6 pt-12 pb-24">
        <header className="mb-12 border-b border-zinc-50 pb-8">
           <p className="text-amber-600 font-black text-[10px] uppercase tracking-[0.2em] mb-3">Legal</p>
        ...
        <article 
          className="prose prose-zinc max-w-none rich-text
            ...
            prose-p:text-zinc-500 prose-p:leading-[1.8] prose-p:text-lg
